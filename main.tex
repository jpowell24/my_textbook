...
(OA.-) to[short] ++(0,-1) coordinate(feedback)

%%%% NPN transistor
(OA.out) ++(1,0) node[npn, ](npn)(T1){}
(OA.out) -- (T1.B)
(T1.C) to[short,-*] ++(0,0)

(T1.C) ++(0,2) coordinate(Vcc)
(Vcc) to[short,*-*] ++(0,1) node[above]{+$10\V$}

(T1.C) to[R, l_=$R_{load}$] (Vcc)
(T1.C) to[short] ++(-1,0)
to [voltmeter] ++(0,2)
to [short] (Vcc)

%%%% PNP transistor
(T1.E) ++(0,-0.5) node[pnp, ](pnp)(T2){}
(T1.E) -- (T2.E)
(T2.B) to[short,-*] ++(-0.5,0) node[above]{+$1\V$}
(T2.C) -| (feedback)
(T2.C) to[R,l=$1\mathrm{k}\Omega$] ++(0,-2)
to ++(0,0) node[ground]{} 
...
unsigned long last_time = 0; // In micros

const long pulse_time = 100;
const long time_between_pulses = 1000;
...
void loop() {
  ...
  if ((current_micros - last_micros >= time_between_pulses) && (!on_off)) {
    last_time = current_time;
    on_off = HIGH;
    pulse_counter++;
    digitalWrite(out_state, on_off);
  }
   if ((current_micros - last_micros >= pulse_time) && (on_off)) {
    last_time = current_time;
    on_off = LOW;
    digitalWrite(out_state, on_off);
  }
