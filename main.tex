...
\draw
#2 node[op amp](#1-OA){\texttt{F1}} (opamp) {}
(opamp.+) node[left] {}
...
to[short] ++(0,1.5)
to[short] ++(-2.4,0) 
(opamp.out) coordinate(#1-in) to[short] ++(0,0)
;
}
...
\draw
#3 coordinate(#4-in) to[short] ++(1,0)
#1 to [battery, l=9 V] ++(0,2)
to [R, l=1 k$\Omega$] ++(2,0)
to [R, l=2 k$\Omega$] ++(0,-2) coordinate(#4-out)
